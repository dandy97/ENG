...
//陀螺仪心跳包
static uint32_t gyro_heartbeat = 0;
//声明tof变量
static tof_can_data_t tof_can_data;
//陀螺仪变量
static gyro_info_t gyro_info;
//声明电机变量
static motor_measure_t motor_chassis[8];
static motor_measure_t motor_lift[3];
//CAN接收中断回调函数
		...
		if(HAL_OK==HAL_RetVal)
		{
			switch (RxMeg.StdId)
			{
				...
				case CAN_3508_M3_ID:
				{
					...
					i = RxMeg.StdId - 0x201;
					//处理电机数据宏函数
					get_motor_measuer(&motor_lift[i], Data, RxMeg.StdId);
					break;
				}
				...
				case 0x300:
				{
					...
					tof_can_data.climb_dis_l = Data[0]<<8 | Data[1];
					if((Data[2]<<8 | Data[3]) < 1000)
					tof_can_data.climb_dis_r = Data[2]<<8 | Data[3];
					break;
				}
				case 0x301:
				{		
					//取弹tof
					if((Data[0]<<8 | Data[1]) < 1000)
					tof_can_data.pinch_dis_l = Data[0]<<8 | Data[1];
					if((Data[2]<<8 | Data[3]) < 1000)
					tof_can_data.pinch_dis_r = Data[2]<<8 | Data[3];
					break;
				}
				case 0x401:
				{
...
const motor_measure_t *get_Lift_Motor_Measure_Point(uint8_t i)
{
	return &motor_lift[(i & 0x03)];//i与0x03只会是0~3，防止数组溢出
}
